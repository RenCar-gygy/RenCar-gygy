# RenCarApp - MVI Mimarisi (Genel Bakış)

> Bu dosya RenCarApp'te MVI (Model-View-Intent) mimarisinin **tek doğruluk kaynağıdır**
...
>
> Referans implementasyon: ilk tamamlanan Auth ekranı (`ui/auth/login/`).
...
Tam ağaç için bkz. [../folder-structure.md](../folder-structure.md).
...
- DI karar geçmişi için bkz. [../decisions.md](../decisions.md).

---

## 6. Backend Henüz Entegre Değilken (Stub Repository)

- Repository **interface** olarak tanımlanır.
- Geçici olarak `Fake<X>Repository` yazılır; ağ davranışı `delay(...)` ile taklit edilir.
